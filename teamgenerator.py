...
class TeamGenerator:
    ...
    async def update_embed(self, mode, player=None, errorstr=''):
        if mode == 'add' or 'rem':
            ...
            emb.add_field(name=f'Players joined: {len(self.players)}', value=f'``` {playerstr[:-2]} ```', inline=False)
        ...
        if mode == 'vote':
            ...
            emb.add_field(name=f'Players voted: {len(self.playerstr)}', value=f'``` {votestr[:-2]} ```', inline=False)
        ...
        if mode == 'error':
            ...
            emb.add_field(name=f'Players joined: {len(self.playerstr)}', value=f'``` {playerstr[:-2]} ```', inline=False)
        ...
        if mode == 'verror':
            ...
            emb.add_field(name='🌉 🢂 Overpass', value=overpass_str)
            emb.add_field(name='🐦 🢂 Ancient', value=ancient_str)
            emb.add_field(name=f'Players voted: {len(self.already_voted)}', value=f'{len(self.voterstr)} ``` {votestr[:-2]} ```', inline=False)
            emb.set_author(name='Map voting', icon_url=str(self.bot.user.avatar_url))
            emb.set_footer(text=f'created by {self.author.name}')
            emb.set_thumbnail(url=player.avatar_url)
            await self.msg.edit(embed=emb)
